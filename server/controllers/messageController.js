    ...
    try {
        ...
        const [messages] = await db.query(
            `SELECT m.*, 
             ...
             LIMIT ? OFFSET ?`,
            [loggedInUserId, otherUserId, otherUserId, loggedInUserId, parseInt(limit), parseInt(offset)]
        );
...
// @desc    Clear entire conversation with another user (soft delete)
...
const clearConversation = async (req, res) => {
    ...
    try {
        ...
        console.log(`🗑️ Cleared conversation between user ${loggedInUserId} and user ${otherUserId}. Affected ${result.affectedRows} messages.`);
...
const editMessage = async (req, res) => {
    ...
    try {
        // 1. First, get the message to verify the sender and check if it's not too old
        const [messages] = await db.query(
            'SELECT sender_id, timestamp FROM messages WHERE id = ?', 
            [messageId]
        );
        ...
        // 3. Optional: Check if message is too old to edit (e.g., 15 minutes)
        ...
        // 4. If the check passes, update the message
        const [result] = await db.query(
        ...
        // 5. Get the updated message with sender info
...
// @desc    Delete a message sent by the user (soft delete)
    ...
    try {
        ...
        if (messages.length === 0) {
            return res.status(200).json({ 
                success: true,
                message: "Message already deleted or never existed." 
            });
        }
